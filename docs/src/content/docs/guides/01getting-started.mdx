...
The base module (`django_social_auth_rest.urls`) is always required - it exposes [`linked-accounts/`](#whats-next), which works regardless of which providers you've enabled. The provider-specific modules are only needed for providers you've actually configured.
