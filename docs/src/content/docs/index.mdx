...
template: splash
hero:
  tagline: Plug-and-play social login for Django REST Framework. Google & GitHub OAuth, account linking, and JWT tokens - wired up in an afternoon, not a sprint.
  image:
  ...
  actions:
    - text: Get Started
      ...
      icon: rocket
    - text: View on GitHub
      link: https://github.com/krsahil8825/django-social-auth-rest
      icon: github
    - text: View on PyPI
...
<CardGrid>
	...
	<Card title="CSRF-safe by design" icon="approve-check-circle">
		The GitHub flow uses signed, time-boxed OAuth state tokens out of the box, so you're not rolling your own CSRF protection for a third-party redirect.
	</Card>
...
4. **Drop in credentials for the providers you want.**
...
5. **Wire up the URLs and migrate.**
...
No middleware to register, no custom backends to subclass. The full provider-by-provider reference - including callback URLs and credential setup - lives in the [Getting Started guide](/guides/getting-started/).
</Aside>
